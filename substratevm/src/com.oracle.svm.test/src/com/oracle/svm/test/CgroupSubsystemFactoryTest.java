...
public class CgroupSubsystemFactoryTest {
    ...
    public void determineTypeCgroupsV2() {
        try (CgroupMountInfoStub fixture = new CgroupMountInfoStub(MOUNT_INFO_CG2, CGROUPS_CG2)) {
            Path cgroups = fixture.getCgroups();
            Path mountinfo = fixture.getMountInfo();
    ...
    public static class CgroupMountInfoStub implements AutoCloseable {
        ...
        private final Path cgroups;
        private final Path tmpDir;

        public CgroupMountInfoStub(String mountInfoContent, String cgroupsContent) throws IOException {
            Path parent = createTmpDir();
            Path mntInfo = writeTmpFile(parent, "proc-self-mountinfo", mountInfoContent);
            Path cg = writeTmpFile(parent, "proc-cgroups", cgroupsContent);

            this.tmpDir = parent;
            this.mountInfo = mntInfo;
            this.cgroups = cg;
        }
        ...
        private static Path createTmpDir() throws IOException {
            return Files.createTempDirectory(CgroupSubsystemFactoryTest.class.getSimpleName());
        }

        private static Path writeTmpFile(Path parent, String name, String content) throws IOException {
            Path file = Files.createFile(Paths.get(parent.toString(), name));
