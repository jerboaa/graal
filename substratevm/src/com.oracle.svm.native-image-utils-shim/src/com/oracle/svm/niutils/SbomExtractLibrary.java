...
public class SbomExtractLibrary {
    ...
    public static void extractSbom(Path executable) throws SbomExtractionException {
        CCharPointerHolder exe = CTypeConversion.toCString(executable.toAbsolutePath().toString());
        if (extractSbomNative(exe.get()) != 0) {
            throw new SbomExtractionException("Failed to extract SBOM");
        }
    }
