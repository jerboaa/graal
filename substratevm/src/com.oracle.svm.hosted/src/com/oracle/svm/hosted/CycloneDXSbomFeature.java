...
public class CycloneDXSbomFeature implements InternalFeature {
    ...
    public void afterAnalysis(AfterAnalysisAccess access) {
        ...
        final byte[] sbomBytes = sbomBytes(ImageSingletons.lookup(SBomValueWrapper.class).getValue(), access);
    ...
    private static byte[] sbomBytes(String classPathResource, AfterAnalysisAccess access) {
        System.out.println("SBOM classpath resource name: " + classPathResource);
        Class<?> quarkusFeatureClass = null;
        try {
            ClassLoader loader = access.getApplicationClassLoader();
            quarkusFeatureClass = Class.forName("io.quarkus.runner.Feature", true, loader);
        } catch (Exception e) {
            throw VMError.shouldNotReachHere("Must not happen", e);
        }
        try (InputStream in = quarkusFeatureClass.getResourceAsStream(classPathResource)) {
            if (in == null) {
                String msg = String.format("Classpath resource to SBOM file %s does not exist", classPathResource);
                throw VMError.shouldNotReachHere(msg);
            }
            if (!classPathResource.endsWith(".gz")) {
